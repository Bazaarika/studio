...
import { Loader2, PlusCircle, Trash2, GripVertical, Sparkles, Edit, ShoppingBag } from 'lucide-react';
...
export default function CustomizeHomePage() {
    ...
    // Category handlers
    const handleAddCategory = async (name: string, icon: string, imageUrl?: string) => {
        try {
            await addCategory({ name, icon, imageUrl });
    ...
    const handleUpdateCategory = async (id: string, name: string, icon: string, imageUrl?: string) => {
        try {
            await updateCategory(id, { name, icon, imageUrl });
